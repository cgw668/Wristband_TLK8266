 ...
 *
 *
 * START|ID(W)|.ACK|ADDR|.ACK|START|ID(W)|.ACK|.DATA|ACK|.DATA|ACK|......|.DATA|NACK|STOP|
 */
void i2c_burst_read_midstart(u8 id, u16 adr, u8 * buff, int len){
	reg_i2c_id = id;
	i2c_read_buff_midstart((u8)adr, buff, len);
}

void i2c_read_buff_midstart(unsigned short s_addr,unsigned char * pbuf,unsigned int num){
	unsigned char addr_L;
	unsigned int i = 0;
	addr_L = s_addr&0xff;
	/****write address that maybe readed in,frame include:start + id + addr_H + addr_L + stop***/
	write_reg8(0x01,read_reg8(0x01)&0xfe);
	write_reg8(0x05,addr_L);
	write_reg8(0x07,0x15);///start/ID/05(addr_L)
	while(read_reg8(0x02)&0x01);

	/***read data from the address add_H+add_L***/
	write_reg8(0x01,read_reg8(0x01)|0x01);//slave_id | 0x01,.i.e read data. R:High  W:Low

	write_reg8(0x60,read_reg8(0x60)|0x02);////reset
	write_reg8(0x60,read_reg8(0x60)&0xfd);////clear

	write_reg8(0x07,0x11);///start/id
	while(read_reg8(0x02)&0x01);
	if(num>1){
		for(i=0;i<num;i++){
			write_reg8(0x07,0x48);////launch  data read cycle/enable read id...if data is not the last byte readed from slave,master should not ACK to slave,so write 0x48
			while(read_reg8(0x02)&0x01);
			pbuf[i] = read_reg8(0x06);
			if(num==i+2){
				write_reg8(0x07,0xc8);/////if the data is the last byte readed from slave,the master should ACK to slave,so write 0xc8.
				while(read_reg8(0x02)&0x01);
				pbuf[i+1] = read_reg8(0x06);
				break;
			}
		}
	}else{
		write_reg8(0x07,0xc8);
		while(read_reg8(0x02)&0x01);
		pbuf[0] = read_reg8(0x06);
	}
	write_reg8(0x07,0x20);////launch stop cycle
	while(read_reg8(0x02)&0x02);
}
